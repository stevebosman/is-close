...
import 'mocha';
import {isClose, IsCloseScalers} from './index';
...
describe('isClose function test', () => {
    ...
    describe('calculate relative difference using largest of a and b in magnitude - |a-b|/max(|a|,|b|)', () => {
    ...
    describe('calculate relative error using a as theoretical value  - |a-b|/|a|', () => {
        it('should return true when relative tolerance is met (above)', () => {
            const result = isClose(1000, 1010, 0.01, 0, undefined, IsCloseScalers.absA);
            expect(result).to.equal(true);
        });
        it('should return false when relative tolerance is breached (above)', () => {
            const result = isClose(1000, 1010.0000000000001, 0.01, 0, undefined, IsCloseScalers.absA);
            expect(result).to.equal(false);
        });
        it('should return true when relative tolerance is met (below)', () => {
            const result = isClose(1000, 990, 0.01, 0, undefined, IsCloseScalers.absA);
            expect(result).to.equal(true);
        });
        it('should return false when relative tolerance is breached (below)', () => {
            const result = isClose(1000, 989.9999999999999, 0.01, 0, undefined, IsCloseScalers.absA);
            expect(result).to.equal(false);
        });
    });
